...
function validType(type) {
    if (type && _cameraConstants.imageTypes.indexOf(type) === -1) {
        throw new Error('Unrecognized image type of ' + _type + '. Must be one of ' + _cameraConstants.imageTypes.join(", "));
    }
    return true;
}

function validOptions(requestedOptions) {
    var offendingOptions = requestedOptions.reduce(function (opts, option) {
        if (_cameraConstants.options.indexOf(option) === -1) {
            opts.push(option);
        }
        return opts;
    }, []);
    if (offendingOptions.length > 0) {
        throw new Error('Options ' + offendingOptions.join(", ") + ' are not recognized options. Must be one of ' + _cameraConstants.options.join(", "));
    }
    return true;
}

var OSCCommands = function () {
    function OSCCommands(cameraUrl) {
        _classCallCheck(this, OSCCommands);

        this.cameraUrl = cameraUrl;
    }

    _createClass(OSCCommands, [{
        key: '_execute',
        ...
        value: function listAll(entryCount, continuationToken, detail, sort) {
            if (sort && _cameraConstants.sorts.indexOf(sort) === -1) {
                throw new Error('Sort type of ' + sort + ' is unrecognized. Must be one of ' + _cameraConstants.sorts.join(", "));
            }
            var params = {
                entryCount: entryCount,
                continuationToken: continuationToken,
